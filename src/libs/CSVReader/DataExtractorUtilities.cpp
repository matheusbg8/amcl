...
void extractDataCSV(FILE *source,int *msgID, int *labelID,
...
bool isCommentLine(FILE *file)
{
    ...
    fpos_t pos;
    fgetpos(file , &pos); // Take file position

    char c;
    while(isspace(c = fgetc(file))) // search first character
    {
        if(c=='\n') break;
    }
    ...
    // If the first character is #, it is a comment line
    return c == '#';
}
