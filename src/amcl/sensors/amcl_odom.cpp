...
bool AMCLOdom::UpdateAction(pf_t *pf, AMCLSensorData *data)
{
  ...
  case ODOM_MODEL_UNDEWATER:
  {
    ...
    double dt = data->time - time;
//    ROS_INFO("Time diff %f current time %f", dt, data->time);

    for (int i = 0; i < set->sample_count; i++)
    {
      pf_sample_t* sample = set->samples + i;

      delta_bearing = angle_diff(
                        atan2(ndata->delta.v[1], ndata->delta.v[0]), // Orientation given by translation direction
                                 old_pose.v[2])
                        + sample->pose.v[2];
      ...
      if(dt < 2.0)
      {
        double delta_x = water_current_vx * dt,
               delta_y = water_current_vy * dt,
               half_dx = delta_x/2.0,
               half_dy = delta_y/2.0;
        sample->pose.v[0] += half_dx + pf_ran_gaussian(half_dx*half_dx);
        sample->pose.v[1] += half_dy + pf_ran_gaussian(half_dy*half_dy);
//        ROS_INFO_THROTTLE(0.2,"WC %f %f", delta_x, delta_y);
      }
