...
class AmclNode
{
  ...
  private:
    ...
    double water_current_direction=0.0, // Degrees
           water_current_velocity=0.0; // m/s
        ...
        first_reconfigure_call_(true)
{
  ...
  private_nh_.param("water_current_direction", water_current_direction, 0.0);
  private_nh_.param("water_current_velocity", water_current_velocity, 0.0);
  ...
  else if(tmp_model_type == "underwater")
    odom_model_type_ = ODOM_MODEL_UNDEWATER;
  else
  {
...
void AmclNode::reconfigureCB(AMCLConfig &config, uint32_t level)
{
  ...
  water_current_direction = config.water_current_direction;
  water_current_velocity = config.water_current_velocity;
  ...
  odom_->water_current_vx = water_current_velocity * sin(water_current_direction*M_PI/180.0);
  odom_->water_current_vy = water_current_velocity * cos(water_current_direction*M_PI/180.0);
  ROS_INFO("Water direction %f water velocity %f",water_current_direction,water_current_velocity);
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  else if(pf_init_ && lasers_update_[laser_index])
  {
    ...
    // updated correctly
    odata.delta = delta;
    odata.time = laser_scan->header.stamp.toSec();
  ...
  if(lasers_update_[laser_index])
  {
    ...
    if(!(++resample_count_ % resample_interval_)) // Interval of laser iteration
    {
